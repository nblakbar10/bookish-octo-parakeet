...
def load_model():
  model_dir = r"gs://visitrack_storage/SavedModel" 											##untuk load model
  print(model_dir)
...
def bicycle_detect(image,classes,score,boxes):
    ...
    for i in range(10):
        if(classes[i]==2 and score[i]>0.8):
            ...
            file_name=os.path.join('gs://visitrack_storage/Output_Folder',dt_string+'.jpg') 					##untuk save output gambar
            cv2.imwrite(file_name,image)
def runProgram():
    import cv2
    detection_model = load_model()

    video=cv2.VideoCapture(r'https://github.com/nblakbar10/VisitrackDataset/blob/main/pedestrians.mp4?raw=true')	##untuk ambil video testnya
    while(True):
        try:
            ret,img=video.read()
